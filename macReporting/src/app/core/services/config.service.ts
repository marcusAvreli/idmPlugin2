...
/*function propertyOf(key: string | number | symbol, obj: ConfigService): any {
    return key in obj;
}
*/
...
export class ConfigService implements IHttpConfig {
  ...
  console.log("load:"+JSON.stringify(config));
    ...
    if (config) {
      Object.keys(config).forEach((key: string | number | symbol) => {
	  // propertyOf(key, config)
	  this[key as keyof IHttpConfig] = config[key as keyof IHttpConfig];
	  })
	...
	console.log("hello");
      return this.http.get(config.propertiesFile)
        .pipe(tap(res => {
          Object.keys(res)
            .forEach((key: string | number | symbol) => {
            //  this[key] = res[key];
            });
        }));
    } else {
