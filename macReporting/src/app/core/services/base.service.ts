...
export abstract class BaseService<T> {
 ...
 // const options = this.getOptionsWithToken("token");
   return this.http.get(finalUrl).pipe(map(this.extractData), catchError(this.handleError));
   // return this.http.post(this.getServiceUrl(), data).pipe(catchError(this.handleError));;
...
//https://stackoverflow.com/questions/37208801/property-map-does-not-exist-on-type-observableresponse
  ...
  const finalUrl = this.getServiceUrl();
 ...
 protected handleError(error: HttpErrorResponse) {
   ...
   * @returns gets the response
   */
  public update(fieldId: string, data: any): Observable<any> {
  const options = this.getOptionsWithToken("token");
    return this.http.put(this.getServiceUrl() + '/' + data[fieldId as string], JSON.stringify(data), options ).pipe(catchError(this.handleError));
  }
